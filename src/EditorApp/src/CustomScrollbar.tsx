...
function CustomScrollbar({ totalLines, visibleLineCount, currentTopLine, onScrollToLine }: CustomScrollbarProps) {
  const trackRef = React.useRef<HTMLDivElement>(null);
  const [isDragging, setIsDragging] = React.useState(false);
  const dragStartRef = React.useRef<{ mouseY: number; startLine: number }>({ mouseY: 0, startLine: 0 });
